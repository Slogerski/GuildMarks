...
public final class CosmicButton extends PressableWidget {
    ...
    public CosmicButton(int x, int y, int width, int height, Text message, Style style, boolean selected, Runnable action) {
    ...
    public static CosmicButton of(int x, int y, int width, int height, String text, Runnable action) {
        return new CosmicButton(x, y, width, height, Text.literal(text), Style.PRIMARY, false, action);
    }
    ...
    @Override protected void renderWidget(DrawContext c, int mouseX, int mouseY, float delta) {
        float target = isHovered() || isFocused() ? 1f : 0f;
        hoverProgress = MathHelper.lerp(Math.min(1f, Math.max(.08f, delta * .22f)), hoverProgress, target);
        ...
        drawScrollableText(c, MinecraftClient.getInstance().textRenderer, 7, textColor);
    }

    private void renderEditorButton(DrawContext c) {
        ...
        drawScrollableText(c, MinecraftClient.getInstance().textRenderer, 5, textColor);
    }
