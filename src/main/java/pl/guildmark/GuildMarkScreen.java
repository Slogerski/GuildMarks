...
public final class GuildMarkScreen extends Screen {
    ...
    private void renderAutoImport(DrawContext c, int x, int y) {
        ...
        c.drawTextWithShadow(textRenderer, Text.literal(tr("HTTPS only. markUrl images are downloaded and stored locally.", "Tylko HTTPS. Grafiki markUrl zostaną pobrane i zapisane lokalnie.")), x, y + 78, 0xFF998BA9);
    }
    ...
    private void downloadMark(String raw) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder(uri).timeout(Duration.ofSeconds(15)).header("Accept", "image/webp,image/png,image/jpeg,image/*").header("User-Agent", "GuildMark/1.0.1 Minecraft/1.21.8").GET().build();
    ...
    private void autoImport() {
        ...
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder(uri).timeout(Duration.ofSeconds(15)).header("Accept", "application/json").header("User-Agent", "GuildMark/1.0.1 Minecraft/1.21.8").GET().build();
            http.sendAsync(request, HttpResponse.BodyHandlers.ofByteArray())
                .thenCompose(response -> prepareAutoImport(http, response))
                .thenAccept(result -> client.execute(() -> { if (requestId == autoImportRequest) commitAutoImport(result); }))
                .exceptionally(error -> { client.execute(() -> { if (requestId == autoImportRequest) status = tr("Auto Import error: ", "Błąd Auto Import: ") + shorten(rootMessage(error), 52); }); return null; });
        ...
        try {
            ...
            for (GuildData.Guild guild : incoming.guilds) {
                ...
                HttpRequest markRequest = HttpRequest.newBuilder(markUri).timeout(Duration.ofSeconds(20))
                    ...
                    .header("User-Agent", "GuildMark/1.0.1 Minecraft/1.21.8").GET().build();
