...
public final class GuildPlayerPreviewCache {
    ...
    public static LivingEntity get(String nickname) {
        ...
        for (AbstractClientPlayerEntity player : world.getPlayers()) {
            if (player.getGameProfile().getName().equalsIgnoreCase(nickname)) return player;
        }
        ...
        if (!entry.requested) {
            ...
            fetchProfile(nickname).thenAccept(profile -> {
                ...
                client.execute(() -> {
                    ...
                    var supplier = client.getSkinProvider().getSkinTexturesSupplier(profile);
    ...
    private static Entry createFallback(ClientWorld world, String nickname) {
        ...
        return new Entry(new GuildPreviewPlayer(world, profile, () -> DefaultSkinHelper.getSkinTextures(profile)));
    }
    ...
    private static java.util.concurrent.CompletableFuture<GameProfile> fetchProfile(String nickname) {
        ...
        HttpRequest request = HttpRequest.newBuilder(URI.create("https://api.mojang.com/users/profiles/minecraft/" + encoded))
            .timeout(Duration.ofSeconds(8)).header("Accept", "application/json").header("User-Agent", "GuildMark/1.1.1").GET().build();
